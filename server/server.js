 ...
 */
 ...
 function requestHandler(request, response) {
   //resolve the path tot he requested resource and assign it to a variable
   let requestedResource = path.join(
     __dirname, // current directory where server.js is found
     '../public', // step out of this directory into public
   ...
   fs.exists(requestedResource, function(exists) {
     //check if file doesn't exist and return 404 status (File not found)
     if(!exists) {
     ...
     //Finally, we read the reqeusted file (async) and send its content to the client
     fs.readFile(
       requestedResource, //path to requested requestedResource
       "binary", // read the requested resource as a binary File
       function(err, file) {  // callback function to handle end of file reading
 ...
 //setup our server to start listening on port specified
 server.listen(portNumber, function () {
   // log to our console, so we know server is running
   console.log(`Server listening and running on port ${portNumber}`);
 });
